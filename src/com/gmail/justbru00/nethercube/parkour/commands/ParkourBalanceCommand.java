...
public class ParkourBalanceCommand implements CommandExecutor {

	@SuppressWarnings("deprecation")
	@Override
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		
		if (command.getName().equalsIgnoreCase("parkourbalance")) {
			if (sender.hasPermission("nethercubeparkour.parkourbalance")) {
				if (sender instanceof Player) {
					Player p = (Player) sender;
					
					if (args.length == 1) {
						// Trying to use /elybal <player>
						if (sender.hasPermission("nethercubeparkour.parkourbalance.others")) {
							...
							Set<String> keys = NetherCubeParkour.dataFile.getKeys(false);
