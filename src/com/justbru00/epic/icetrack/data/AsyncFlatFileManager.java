...
public class AsyncFlatFileManager {
	...
	public static void init() {
		if (repeatingTaskId == 0) {
			BukkitTask task = Bukkit.getScheduler().runTaskTimerAsynchronously(EpicIceTrack.getInstance(),
					new Runnable() {
	...
	/**
	 * This method should be run async as it both reads and possibly saves a file
	...
	public static void onPlayerJoin(UUID playerUuid) {
		...
		if (possiblePdf.isEmpty()) {
			...
			Messager.debug(
					"[AsyncFlatFileManager#onPlayerJoin] We couldn't find the cached file for this player. That shouldn't be possible as we have just created one.");
			return;
		}
		...
		if (pdf.getString("lastname") == null) {
			...
			pdf.set("currency", 0);
			// Map data storage keys are abbreviated to save storage space for large amounts
			// of players.
			// u = unlocked
			// a = attempts
			// f = finishes
	...
	private static void runEvery30SecondsAsync() {
		...
		for (AsyncCachedPluginFile f : cachedFiles) {
			if (f.isSaveNeeded()) {
				...
				f.getFile().save();
				Messager.msgConsole("&7[AsyncFlatFileManager] Saved player data to disk for " + f.getPlayerUuid()
						+ " in " + Duration.between(start, Instant.now()).toMillis() + " ms.");
	...
	private static Optional<PlayerData> getPlayerDataFromCachedFile(UUID playerUuid) {
		...
		if (cachedFile.isEmpty()) {
			// This shouldn't happen ever
			Messager.debug(
					"[AsyncFlatFileManager#getPlayerDataFromCachedFile] We couldn't find the cached file for this player."
	...
	public static void loadAndCacheFile(UUID playerUuid) {
		...
		File playersDataFile = new File(EpicIceTrack.getInstance().getDataFolder().getPath() + File.separator
				+ "flatfilestorage" + File.separator + playerUuid.toString() + ".yml");
